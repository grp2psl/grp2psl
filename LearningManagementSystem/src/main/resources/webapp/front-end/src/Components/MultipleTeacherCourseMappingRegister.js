...
class MultipleTeacherCourseMappingRegister extends React.Component{	
    ...
    async download(){
		...
		try{
			const response = await axios.get(DATABASE_URL+MANAGER_URL+TCMAPPING_URL+"/generate-excel", {
                auth: {
    ...
    async assignTrainers(){
        ...
        if(this.state.file != null){
            ...
            try{
                const response = await axios.post(DATABASE_URL+MANAGER_URL+TCMAPPING_URL+"/register-multiple", formData, {
                    auth: {
                      username: localStorage.getItem("username"),
                      password: localStorage.getItem("password")
                  },
