...
export default class UpdateAdminPassword extends Component{
	constructor(props){
		super(props);
	...
	async sendPassword(event){
		...
		try{ 
			...
			if(this.state.newpassword===this.state.confirmpassword){
				const response = await axios.put(DATABASE_URL+MANAGER_URL+"/updateadmin/" + localStorage.getItem("userId")+"/currentPassword/"+this.state.currentPassword+"/newPassword/"+this.state.newpassword, 
			{},{auth: {
